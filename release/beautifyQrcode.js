...
(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined'
        ...
        : ((global =
          ...
          factory(
              (global.beautifyQrcode = {}),
              null,
              global.yup,
              global.renderer,
              null,
              global.qrcodeHandler,
              global.util,
              null,
              null,
              null,
    ...
    function listPoints(qrcode, params) {
        ...
        var typeTable = qrcodeHandler.getTypeTable(qrcode);
        ...
        for (var x = 0; x < nCount; x++) {
            for (var y = 0; y < nCount; y++) {
                ...
                if (
                    typeTable[x][y] ===
                        qrcodeHandler.QRPointType.ALIGN_CENTER ||
                    typeTable[x][y] === qrcodeHandler.QRPointType.ALIGN_OTHER ||
                    typeTable[x][y] === qrcodeHandler.QRPointType.TIMING
                ) {
                    if (type === 0)
                        pointList.push(
                            '<rect\n                            opacity="'
                    ...
                    typeTable[x][y] === qrcodeHandler.QRPointType.POS_CENTER
                ) {
                    if (posType === 0) {
                        pointList.push(
                            '<rect\n                            width="'
                    ...
                    typeTable[x][y] === qrcodeHandler.QRPointType.POS_OTHER
                ) {
                    if (posType === 0) {
                        pointList.push(
                            '<rect\n                            width="'
    ...
    var rendererBase = function rendererBase(qrcode, options) {
        ...
        var svg = renderer.createRenderer({
        ...
        for (var _y = 0; _y < nCount; _y++) {
            for (var _x = 0; _x < nCount; _x++) {
                if (qrcode.isDark(_x, _y) === false) continue;
                    ...
                    typeTable[_x][_y] === qrcodeHandler.QRPointType.POS_CENTER
                ) {
                    if (posType === 0) {
                        pointList.push(
                            '<rect\n                            width="'
                    ...
                    typeTable[_x][_y] === qrcodeHandler.QRPointType.POS_OTHER
                ) {
                    if (posType === 0) {
                        pointList.push(
                            '<rect\n                            width="'
    ...
    var schemaDSJ = yup.object().shape({
        // 信息点缩放
    ...
    var RenderDSJ = function RenderDSJ(qrcode, options) {
        ...
        var params = ['width2', 'width1', 'width3', 'posType'].map(
            function (k) {
                _newArrowCheck(this, _this2);
    ...
    function listPoints$2(qrcode, params) {
        ...
        for (var i = 0; i < randArr.length; i++) {
            ...
            if (qrcode.isDark(_row, _col)) {
                var tempRand = util.rand(0.8, 1.3);
                var randNum = util.rand(50, 230);
                var tempRGB = [
                    'rgb(' +
    ...
    var schema25D = yup.object().shape({
        // 柱体高度
    ...
    var schemaImage = yup.object().shape({
        // 背景图片
                    ...
                    .then(
                        function (gpl) {
                            ...
                            var svg = '<svg\n            className="Qr-item-svg"\n            width="'
                                ...
                                .concat(
                                    gpl
                                        .concat(listPoints$5(qrcode, params))
                                        .join(''),
                                    '\n        </svg>'
                                );
                            resolve(svg);
    ...
    var schemaResImage = yup.object().shape({
        // 背景图片
                ...
                if (
                    ...
                    if (available[_x][_y]) {
                        if (qrcode.isDark(_x, _y)) {
                            ...
                            typeTable[_x][_y] === qrcodeHandler.QRPointType.DATA
                        ) {
                            if (util.rand(0, 1) > 0.85) {
                                g2.push(
                                    '<circle\n                                    r="'
                                        .concat(
                                            0.5 * util.rand(0.85, 1.3),
                                            '"\n                                    key="'
                                        )
                                        .concat(
                                            id++,
    ...
    var schemaBase$1 = yup.object().shape({
    ...
    var rendererCircle = function rendererCircle(qrcode, options) {
        ...
        var params = ['otherColor', 'posColor'].map(
            function (k) {
                _newArrowCheck(this, _this2);
    ...
    var schemaFuncA = yup.object().shape({
    ...
    var schemaFuncB = yup.object().shape({
        type: yup.mixed().oneOf([0, 1])['default'](1),
    ...
    var schemaLine = yup.object().shape({
    ...
    var schemaLine2 = yup.object().shape({
